...
/*   Updated: 2021/11/20 20:54:05 by isoria-g         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_words(t_split *split, char const *s1, char c)
{
	...
	while (s1[split->i])
	{
		...
		if (split->i != 0 && (s1[split->i] == c && s1[split->i + 1] == c
				&& s1[split->i - 1] != '\\' && split->f_simple == 0
				&& split->f_double == 0))
			split->j = 0;
		else if (split->i == 0 && s1[split->i] == c && s1[split->i + 1] == c
...
static char	**ft_divide(t_split *split, char const *s, char **str, char c)
{
	...
	while (s[split->i] != '\0' && split->j < split->n && split->n > 1)
	{
		split->k = 0;
		while (s[split->i] == c || (s[split->i] == c && (split->f_double != 0
					|| split->f_simple != 0)) || (s[split->i] == c
				&& s[split->i - 1] == '\\'))
		{
		...
		while (check_condition(split, s, c))
		{
